...
export const exerciseService = {
  ...
  async get(filter: IExerciseFilter): Promise<Array<IExerciseDTO>> {
    const edits = await storageService.get<IExerciseEditDTO>(
      "exercise",
      filter
    );
    return await editDtoToDto(edits);
  },
  ...
  async save(dto: IExerciseDTO): Promise<IExerciseEditDTO> {
    const dtoToSave = dtoToEditDto(dto);
    const editDto = dtoToSave.id
      ? await storageService.put<IExerciseEditDTO>("exercise", dtoToSave)
      : await storageService.post<IExerciseEditDTO>("exercise", dtoToSave);
    //Temp till connected to backend
    const x = await editDtoToDto([editDto]);
    return x[0];
  },
...
const dtoToEditDto = (dto: IExerciseDTO): IExerciseEditDTO => {
  return {
    id: dto.id,
    name: dto.name,
